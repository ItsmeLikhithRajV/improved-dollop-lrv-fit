/**
 * CORE UI PRIMITIVES
 * Base building blocks for the Sentient UI system.
 */
...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(({ className, variant = "default", size = "default", ...props }, ref) => {
    ...
    return (
        <button
            ref={ref}
            className={cn(
                "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-lg font-medium transition-all duration-300 focus-visible:outline-none disabled:opacity-50 disabled:pointer-events-none press-effect hover-glow",
                variants[variant],
                sizes[size],
                className
            )}
...
export const GlassCard: React.FC<GlassCardProps> = ({ children, className, onClick, tier = 2 }) => {
    const tierClass = `glass-tier-${tier}`;

    return (
        <motion.div
            onClick={onClick}
            className={cn(tierClass, "rounded-xl p-6 relative overflow-hidden", className)}
